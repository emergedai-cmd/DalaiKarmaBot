...
## DalaiKarmaBot (hardened fork)

This zip includes a small fork layer that:
- adds a `dalaikarmabot` CLI entrypoint (wrapper around upstream runtime)
- provides a **secure-by-default** config profile and local-LLM templates under `examples/`
...
- Start from the secure profile:
  - `examples/dalaikarmabot-secure.profile.json5`
- Add one local model provider:
  - `examples/local-models/lmstudio.provider.json5` **or** `examples/local-models/ollama.provider.json5`
...
```bash
pnpm dalaikarmabot -- gateway
# or
node dalaiKarmaBot.mjs gateway
```

**Security note:** The gateway will refuse to bind to a non-loopback interface unless you configure token/password auth.
