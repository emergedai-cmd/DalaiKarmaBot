...
### Added
- First real runtime command: `dalaikarmabot run`
  - Supports local Ollama and hosted providers
  - Providers: ollama, groq, openrouter, deepseek, mistral
- Security-focused onboarding during `init`
  - Clear env var instructions (temporary vs persistent)
  - OS-aware guidance (Windows, Linux, Docker)
  - Copy/paste-ready example commands
- Enhanced `doctor` command
  - Validates local endpoints (Ollama, LM Studio, n8n)
  - Checks presence of hosted provider API keys
  - Optional `/models` connectivity check via `--models`
  - CI-friendly flags: `--json`, `--quiet`, `--strict`
- Full transparency tooling
  - `where` and `explain` document filesystem and network access
  - `--dry-run` supported across init paths

### Security
- No secrets written to disk (env vars only)
- No implicit LLM calls in diagnostics
- Minimal published build surface preserved
