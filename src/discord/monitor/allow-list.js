...
function resolveDiscordUserAllowed(params) {
    var allowList = normalizeDiscordAllowList(params.allowList, ["discord:", "user:", "pk:"]);
    if (!allowList) {
        return true;
    }
...
function resolveDiscordCommandAuthorized(params) {
    ...
    var allowList = normalizeDiscordAllowList(params.allowFrom, ["discord:", "user:", "pk:"]);
    if (!allowList) {
        return true;
    }
    return allowListMatches(allowList, {
...
function resolveGroupDmAllow(params) {
    ...
    var allowList = new Set(channels.map(function (entry) { return normalizeDiscordSlug(String(entry)); }));
    var candidates = [
        normalizeDiscordSlug(channelId),
        channelSlug,
        channelName ? normalizeDiscordSlug(channelName) : "",
    ].filter(Boolean);
    return allowList.has("*") || candidates.some(function (candidate) { return allowList.has(candidate); });
}
function shouldEmitDiscordReactionNotification(params) {
    ...
    if (mode === "allowlist") {
        var list = normalizeDiscordAllowList(params.allowlist, ["discord:", "user:", "pk:"]);
        if (!list) {
            return false;
        }
