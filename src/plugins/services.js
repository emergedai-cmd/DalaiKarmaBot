"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.startPluginServices = startPluginServices;
var paths_js_1 = require("../config/paths.js");
var subsystem_js_1 = require("../logging/subsystem.js");
var log = (0, subsystem_js_1.createSubsystemLogger)("plugins");
function startPluginServices(params) {
    return __awaiter(this, void 0, void 0, function () {
        var running, _loop_1, _i, _a, entry;
        var _this = this;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    running = [];
                    _loop_1 = function (entry) {
                        var service, err_1;
                        return __generator(this, function (_c) {
                            switch (_c.label) {
                                case 0:
                                    service = entry.service;
                                    _c.label = 1;
                                case 1:
                                    _c.trys.push([1, 3, , 4]);
                                    return [4 /*yield*/, service.start({
                                            config: params.config,
                                            workspaceDir: params.workspaceDir,
                                            stateDir: paths_js_1.STATE_DIR,
                                            logger: {
                                                info: function (msg) { return log.info(msg); },
                                                warn: function (msg) { return log.warn(msg); },
                                                error: function (msg) { return log.error(msg); },
                                                debug: function (msg) { return log.debug(msg); },
                                            },
                                        })];
                                case 2:
                                    _c.sent();
                                    running.push({
                                        id: service.id,
                                        stop: service.stop
                                            ? function () {
                                                var _a;
                                                return (_a = service.stop) === null || _a === void 0 ? void 0 : _a.call(service, {
                                                    config: params.config,
                                                    workspaceDir: params.workspaceDir,
                                                    stateDir: paths_js_1.STATE_DIR,
                                                    logger: {
                                                        info: function (msg) { return log.info(msg); },
                                                        warn: function (msg) { return log.warn(msg); },
                                                        error: function (msg) { return log.error(msg); },
                                                        debug: function (msg) { return log.debug(msg); },
                                                    },
                                                });
                                            }
                                            : undefined,
                                    });
                                    return [3 /*break*/, 4];
                                case 3:
                                    err_1 = _c.sent();
                                    log.error("plugin service failed (".concat(service.id, "): ").concat(String(err_1)));
                                    return [3 /*break*/, 4];
                                case 4: return [2 /*return*/];
                            }
                        });
                    };
                    _i = 0, _a = params.registry.services;
                    _b.label = 1;
                case 1:
                    if (!(_i < _a.length)) return [3 /*break*/, 4];
                    entry = _a[_i];
                    return [5 /*yield**/, _loop_1(entry)];
                case 2:
                    _b.sent();
                    _b.label = 3;
                case 3:
                    _i++;
                    return [3 /*break*/, 1];
                case 4: return [2 /*return*/, {
                        stop: function () { return __awaiter(_this, void 0, void 0, function () {
                            var _i, _a, entry, err_2;
                            return __generator(this, function (_b) {
                                switch (_b.label) {
                                    case 0:
                                        _i = 0, _a = running.toReversed();
                                        _b.label = 1;
                                    case 1:
                                        if (!(_i < _a.length)) return [3 /*break*/, 6];
                                        entry = _a[_i];
                                        if (!entry.stop) {
                                            return [3 /*break*/, 5];
                                        }
                                        _b.label = 2;
                                    case 2:
                                        _b.trys.push([2, 4, , 5]);
                                        return [4 /*yield*/, entry.stop()];
                                    case 3:
                                        _b.sent();
                                        return [3 /*break*/, 5];
                                    case 4:
                                        err_2 = _b.sent();
                                        log.warn("plugin service stop failed (".concat(entry.id, "): ").concat(String(err_2)));
                                        return [3 /*break*/, 5];
                                    case 5:
                                        _i++;
                                        return [3 /*break*/, 1];
                                    case 6: return [2 /*return*/];
                                }
                            });
                        }); },
                    }];
            }
        });
    });
}
