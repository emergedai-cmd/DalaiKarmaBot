...
function getHeadersWithAuth(url, headers) {
    ...
    var relayHeaders = (0, extension_relay_js_1.getChromeExtensionRelayAuthHeaders)(url);
    var mergedHeaders = __assign(__assign({}, relayHeaders), headers);
    try {
        var parsed = new URL(url);
        var hasAuthHeader = Object.keys(mergedHeaders).some(function (key) { return key.toLowerCase() === "authorization"; });
        if (hasAuthHeader) {
            return mergedHeaders;
        }
        if (parsed.username || parsed.password) {
            var auth = Buffer.from("".concat(parsed.username, ":").concat(parsed.password)).toString("base64");
            return __assign(__assign({}, mergedHeaders), { Authorization: "Basic ".concat(auth) });
        }
    }
    catch (_a) {
        // ignore
