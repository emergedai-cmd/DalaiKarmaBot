...
exports.DEFAULT_TAGLINE = exports.HOLIDAY_RULES = exports.TAGLINES = void 0;
exports.activeTaglines = activeTaglines;
exports.pickTagline = pickTagline;
var DEFAULT_TAGLINE = "All your chats, one OpenClaw.";
exports.DEFAULT_TAGLINE = DEFAULT_TAGLINE;
...
var onMonthDay = function (month, day) {
    return function (date) {
        var parts = utcParts(date);
        return parts.month === month && parts.day === day;
    };
};
var onSpecificDates = function (dates, durationDays) {
    if (durationDays === void 0) { durationDays = 1; }
    return function (date) {
        var parts = utcParts(date);
        return dates.some(function (_a) {
            var year = _a[0], month = _a[1], day = _a[2];
            if (parts.year !== year) {
                return false;
            }
            var start = Date.UTC(year, month, day);
            var current = Date.UTC(parts.year, parts.month, parts.day);
            return current >= start && current < start + durationDays * DAY_MS;
        });
    };
};
var inYearWindow = function (windows) {
    return function (date) {
        var parts = utcParts(date);
        var window = windows.find(function (entry) { return entry.year === parts.year; });
        if (!window) {
            return false;
        }
        var start = Date.UTC(window.year, window.month, window.day);
        var current = Date.UTC(parts.year, parts.month, parts.day);
        return current >= start && current < start + window.duration * DAY_MS;
    };
};
var isFourthThursdayOfNovember = function (date) {
    var parts = utcParts(date);
    if (parts.month !== 10) {
        return false;
    } // November
    var firstDay = new Date(Date.UTC(parts.year, 10, 1)).getUTCDay();
    var offsetToThursday = (4 - firstDay + 7) % 7; // 4 = Thursday
    var fourthThursday = 1 + offsetToThursday + 21; // 1st + offset + 3 weeks
    return parts.day === fourthThursday;
};
var HOLIDAY_RULES = new Map([
...
exports.HOLIDAY_RULES = HOLIDAY_RULES;
function isTaglineActive(tagline, date) {
    var rule = HOLIDAY_RULES.get(tagline);
    if (!rule) {
        return true;
    }
    return rule(date);
...
function activeTaglines(options) {
    ...
    var today = options.now ? options.now() : new Date();
    var filtered = TAGLINES.filter(function (tagline) { return isTaglineActive(tagline, today); });
...
function pickTagline(options) {
    ...
    var override = env === null || env === void 0 ? void 0 : env.OPENCLAW_TAGLINE_INDEX;
    if (override !== undefined) {
        var parsed = Number.parseInt(override, 10);
        if (!Number.isNaN(parsed) && parsed >= 0) {
            var pool_1 = TAGLINES.length > 0 ? TAGLINES : [DEFAULT_TAGLINE];
            return pool_1[parsed % pool_1.length];
        }
    }
    var pool = activeTaglines(options);
    var rand = (_b = options.random) !== null && _b !== void 0 ? _b : Math.random;
    var index = Math.floor(rand() * pool.length) % pool.length;
    return pool[index];
}
