...
function isSensitivePath(path) {
    return SENSITIVE_PATTERNS.some(function (pattern) { return pattern.test(path); });
}
...
function mergeObjectSchema(base, extension) {
    var _a, _b, _c;
    var mergedRequired = new Set(__spreadArray(__spreadArray([], ((_a = base.required) !== null && _a !== void 0 ? _a : []), true), ((_b = extension.required) !== null && _b !== void 0 ? _b : []), true));
    var merged = __assign(__assign(__assign({}, base), extension), { properties: __assign(__assign({}, base.properties), extension.properties) });
    if (mergedRequired.size > 0) {
        merged.required = Array.from(mergedRequired);
    }
    var additional = (_c = extension.additionalProperties) !== null && _c !== void 0 ? _c : base.additionalProperties;
...
function buildBaseHints() {
    var hints = {};
    for (var _i = 0, _a = Object.entries(GROUP_LABELS); _i < _a.length; _i++) {
        var _b = _a[_i], group = _b[0], label = _b[1];
        hints[group] = {
            label: label,
            group: label,
    ...
    for (var _c = 0, _d = Object.entries(FIELD_LABELS); _c < _d.length; _c++) {
        var _e = _d[_c], path = _e[0], label = _e[1];
        var current = hints[path];
        hints[path] = current ? __assign(__assign({}, current), { label: label }) : { label: label };
    }
    for (var _f = 0, _g = Object.entries(FIELD_HELP); _f < _g.length; _f++) {
        var _h = _g[_f], path = _h[0], help = _h[1];
        var current = hints[path];
        hints[path] = current ? __assign(__assign({}, current), { help: help }) : { help: help };
    }
    for (var _j = 0, _k = Object.entries(FIELD_PLACEHOLDERS); _j < _k.length; _j++) {
        var _l = _k[_j], path = _l[0], placeholder = _l[1];
        var current = hints[path];
        hints[path] = current ? __assign(__assign({}, current), { placeholder: placeholder }) : { placeholder: placeholder };
    }
    return hints;
}
function applySensitiveHints(hints) {
    var next = __assign({}, hints);
    for (var _i = 0, _a = Object.keys(next); _i < _a.length; _i++) {
        var key = _a[_i];
        if (isSensitivePath(key)) {
            next[key] = __assign(__assign({}, next[key]), { sensitive: true });
        }
    }
    return next;
}
function applyPluginHints(hints, plugins) {
    ...
    for (var _i = 0, plugins_1 = plugins; _i < plugins_1.length; _i++) {
        ...
        var id = plugin.id.trim();
        if (!id) {
            continue;
        }
        var name_1 = ((_a = plugin.name) !== null && _a !== void 0 ? _a : id).trim() || id;
        var basePath = "plugins.entries.".concat(id);
        next[basePath] = __assign(__assign({}, next[basePath]), { label: name_1, help: plugin.description
                ? "".concat(plugin.description, " (plugin: ").concat(id, ")")
                : "Plugin entry for ".concat(id, ".") });
        next["".concat(basePath, ".enabled")] = __assign(__assign({}, next["".concat(basePath, ".enabled")]), { label: "Enable ".concat(name_1) });
        next["".concat(basePath, ".config")] = __assign(__assign({}, next["".concat(basePath, ".config")]), { label: "".concat(name_1, " Config"), help: "Plugin-defined config payload for ".concat(id, ".") });
        var uiHints = (_b = plugin.configUiHints) !== null && _b !== void 0 ? _b : {};
        for (var _c = 0, _d = Object.entries(uiHints); _c < _d.length; _c++) {
            var _e = _d[_c], relPathRaw = _e[0], hint = _e[1];
            var relPath = relPathRaw.trim().replace(/^\./, "");
            if (!relPath) {
                continue;
            }
            var key = "".concat(basePath, ".config.").concat(relPath);
            next[key] = __assign(__assign({}, next[key]), hint);
        }
    }
    return next;
}
function applyChannelHints(hints, channels) {
    ...
    for (var _i = 0, channels_1 = channels; _i < channels_1.length; _i++) {
        ...
        var basePath = "channels.".concat(id);
        var current = (_a = next[basePath]) !== null && _a !== void 0 ? _a : {};
        var label = (_b = channel.label) === null || _b === void 0 ? void 0 : _b.trim();
        var help = (_c = channel.description) === null || _c === void 0 ? void 0 : _c.trim();
        next[basePath] = __assign(__assign(__assign({}, current), (label ? { label: label } : {})), (help ? { help: help } : {}));
        var uiHints = (_d = channel.configUiHints) !== null && _d !== void 0 ? _d : {};
        for (var _e = 0, _f = Object.entries(uiHints); _e < _f.length; _e++) {
            var _g = _f[_e], relPathRaw = _g[0], hint = _g[1];
            var relPath = relPathRaw.trim().replace(/^\./, "");
            if (!relPath) {
                continue;
            }
            var key = "".concat(basePath, ".").concat(relPath);
            next[key] = __assign(__assign({}, next[key]), hint);
        }
    }
    return next;
...
function listHeartbeatTargetChannels(channels) {
    var seen = new Set();
    var ordered = [];
    for (var _i = 0, CHANNEL_IDS_1 = registry_js_1.CHANNEL_IDS; _i < CHANNEL_IDS_1.length; _i++) {
        var id = CHANNEL_IDS_1[_i];
    ...
    for (var _a = 0, channels_2 = channels; _a < channels_2.length; _a++) {
        ...
        var normalized = channel.id.trim().toLowerCase();
        if (!normalized || seen.has(normalized)) {
            continue;
        }
        seen.add(normalized);
...
function applyHeartbeatTargetHints(hints, channels) {
    var _a, _b, _c;
    var next = __assign({}, hints);
    var channelList = listHeartbeatTargetChannels(channels);
    var channelHelp = channelList.length ? " Known channels: ".concat(channelList.join(", "), ".") : "";
    var help = "Delivery target (\"last\", \"none\", or a channel id).".concat(channelHelp);
    var paths = ["agents.defaults.heartbeat.target", "agents.list.*.heartbeat.target"];
    for (var _i = 0, paths_1 = paths; _i < paths_1.length; _i++) {
        var path = paths_1[_i];
        var current = (_a = next[path]) !== null && _a !== void 0 ? _a : {};
        next[path] = __assign(__assign({}, current), { help: (_b = current.help) !== null && _b !== void 0 ? _b : help, placeholder: (_c = current.placeholder) !== null && _c !== void 0 ? _c : "last" });
    }
    return next;
}
function applyPluginSchemas(schema, plugins) {
    var _a, _b, _c, _d, _e;
    var next = cloneSchema(schema);
    var root = asSchemaObject(next);
    var pluginsNode = asSchemaObject((_a = root === null || root === void 0 ? void 0 : root.properties) === null || _a === void 0 ? void 0 : _a.plugins);
    var entriesNode = asSchemaObject((_b = pluginsNode === null || pluginsNode === void 0 ? void 0 : pluginsNode.properties) === null || _b === void 0 ? void 0 : _b.entries);
    if (!entriesNode) {
        return next;
    }
    var entryBase = asSchemaObject(entriesNode.additionalProperties);
    var entryProperties = (_c = entriesNode.properties) !== null && _c !== void 0 ? _c : {};
    ...
    for (var _i = 0, plugins_2 = plugins; _i < plugins_2.length; _i++) {
        ...
        var entrySchema = entryBase
            ? cloneSchema(entryBase)
            : { type: "object" };
        var entryObject = (_d = asSchemaObject(entrySchema)) !== null && _d !== void 0 ? _d : { type: "object" };
        var baseConfigSchema = asSchemaObject((_e = entryObject.properties) === null || _e === void 0 ? void 0 : _e.config);
        var pluginSchema = asSchemaObject(plugin.configSchema);
        var nextConfigSchema = baseConfigSchema &&
            pluginSchema &&
            isObjectSchema(baseConfigSchema) &&
            isObjectSchema(pluginSchema)
            ? mergeObjectSchema(baseConfigSchema, pluginSchema)
            : cloneSchema(plugin.configSchema);
        entryObject.properties = __assign(__assign({}, entryObject.properties), { config: nextConfigSchema });
...
function applyChannelSchemas(schema, channels) {
    ...
    var next = cloneSchema(schema);
    var root = asSchemaObject(next);
    var channelsNode = asSchemaObject((_a = root === null || root === void 0 ? void 0 : root.properties) === null || _a === void 0 ? void 0 : _a.channels);
    if (!channelsNode) {
        return next;
    }
    var channelProps = (_b = channelsNode.properties) !== null && _b !== void 0 ? _b : {};
    ...
    for (var _i = 0, channels_3 = channels; _i < channels_3.length; _i++) {
        ...
        var existing = asSchemaObject(channelProps[channel.id]);
        var incoming = asSchemaObject(channel.configSchema);
...
function stripChannelSchema(schema) {
    ...
    var channelsNode = asSchemaObject(root.properties.channels);
    if (channelsNode) {
...
function buildBaseConfigSchema() {
    ...
    var schema = zod_schema_js_1.OpenClawSchema.toJSONSchema({
        target: "draft-07",
        unrepresentable: "any",
    });
    schema.title = "OpenClawConfig";
    var hints = applySensitiveHints(buildBaseHints());
    var next = {
...
function buildConfigSchema(params) {
    var _a, _b;
    var base = buildBaseConfigSchema();
    var plugins = (_a = params === null || params === void 0 ? void 0 : params.plugins) !== null && _a !== void 0 ? _a : [];
    var channels = (_b = params === null || params === void 0 ? void 0 : params.channels) !== null && _b !== void 0 ? _b : [];
    if (plugins.length === 0 && channels.length === 0) {
        return base;
    }
    var mergedHints = applySensitiveHints(applyHeartbeatTargetHints(applyChannelHints(applyPluginHints(base.uiHints, plugins), channels), channels));
    var mergedSchema = applyChannelSchemas(applyPluginSchemas(base.schema, plugins), channels);
    return __assign(__assign({}, base), { schema: mergedSchema, uiHints: mergedHints });
}
